class ProductResultLanguage{
  // Asisinde/Bhula Sangoma
  static const imiphumelaAsisinde = [
    ['Sigeza Amathunzi Amnyama'],
    ['Sihlehlisa Izitha'],
    ['Sicacisa Amaphupho']
  ];

  // Asiphephe/Impendulo
  static const imiphumelaAsiphephe = [
    ['Sibuyisela Okubi Emuva'],
    ['Silwa Nezichitho'],
    ['Silwa Namalumbo']
  ];

  // Mabeze/Umazibuthe
  static const imiphumelaMabeze = [
    ['Silanda Amacustomer'],
    ['Siqeda Ubukhuphe'],
    ['Sikwenza Uthandeke'],
  ];

  // Siyamthanda/Nobuhle
  static const imiphumelaSyamthanda = [
    ['Siletha Ukuthandeka'],
    ['Senza Bachazeke Uwena'],
    ['Sivula Inhlanhla'],
  ];

  // My No1/Isimomondiya
  static const imiphumelaMyNo1 = [
    ['Sikwenza Ugqame Kahle Emsebenzini Nasebantwini.'],
    ['Senza Bakhulume Ngawe Kahle Yonke Indawo.'],
    ['Iphutha Lakho Alinakeki.'],
  ];

  // Umakoti/Baganise
  static const imiphumelaUmakotiLo = [
    ['Indoda Ikuphathisa Okweqanda.'],
    ['Uba Yinkanyezi Kwabesilisa.'],
    ['Ungahlawulwa, Ungathola Nomendo.'],
  ];

  // Umhloniphe/Yellow Bone
  static const imiphumelaMhloniphe = [
    ['Uba Nesithunzi'],
    ['Uyahlonipheka'],
    ['Ungagcina Usabeka Mawugquma Ngaphezulu Kwezinsuku Ezinhlanu.'],
  ];

  // Siyakvumela/Fodo Finish
  static const imiphumelaSiyakuvumela = [
    ['Senza Ulaleleke'],
    ['Silanda Abathengi'],
    ['Sigeza Idlozi'],
  ];

  // Madida/Mpukane
  static const imiphumelaMadida = [
    ['Bayakubanga'],
    ['Bayakulandela Bonke'],
    ['Ubanesicefe Esimnandi']
  ];

  static const imiphumelaEsodazi = [
    ['Bayakubanga'],
    ['Bayakulandela Bonke'],
    ['Ubanesicefe Esimnandi']
  ];

  static const imiphumelaSukadeda = [
    ['Sixosha Zonke Izilwane Zomthakathi, Umkhovu, Isikhova, Kanye Nezinye.',],
    ['Siphuphisa Konke Okuyisandla Somthakathi Egcekeni, Umeqo,  Ischitho. ',],
    ['Okuthakathayo Uyaziveza Mengabe Eseduze Njengo Umakhelwane.',]
  ];

  static const imiphumelaThengani =
  ['Wenza Abantu Bathenge Ubuthaphuthaphu. Ulungele Umuthu Odayisa Lapho Kudlula Khona Abantu.',
  ];

  static const imiphumelaQinanduku = 
  ['Uqinisa Induku Kowesilisa',];

  static const imiphumelaOwemfuyo = 
  ['Wenza izimbuzi, nezikabhu zizale amawele.',];

  static const imiphumelaMehlothando =
  ['Uma Umbuka Emehlweni Umuntu Kwakheka Uthando Lwakho Kuye. Noma Ngabe Umphathi Wakho, Umuntu Omfunayo, Ngisho Nesitha Sakho.',
  ];

  static const imiphumelaNgphuphe =
  ['Susa Isinyama Kucace Amaphupho.',
  ];

  static const imiphumelaUkuzuqashwe =
  ['Uwuthola Kumnyama Kubovu Umsebenzi Ngalenduku.',
  ];

  static const imiphumelaGezaUmsamo =
  ['Ugeza Idlozi Lakho Likwazi Ukukhuluma Likucacisele Okufanele Ukwenze.',
  ];

  static const imiphumelaAkabuye =
  ['Uyabuyisa',
  ];

  static const imiphumelaVulidlozi = 
  ['Kuvuleka Udlozi Umakade Livalekile. Ngokwenza Loku Kuqaqeka Izinto Zakho Ngoba Idlozi Lisuke Selithokozile.',
  ];

   static const imiphumelaBelungubami = 
  ['Abaphathi Bakho Emsebenzi Bayakukhonza, Bakuhloniphe, Babenomusa Omkhulu Kuwe, Ungathola Nesikhundla.',
  ];

  static const imiphumelaDonsamali = 
  ['Ulanda Imali, Ubamba Imali Ingabaleki.',
  ];
  static const imiphumelaHlanzisinya = 
  ['Ugeza Isinya Sakho, Uvule Nama Payipi Esinya Avalekile.',
  ];

  static const imiphumelaNyokaEsiswini = 
  ['Uqenda Ilumbo Lenyoka Ehamba Esiswini.',
  ];

  static const imiphumelaMndenongaxabani =
  ['Emndenini Noma Kulabo Abahlala Endlini Eyodwa Kuba Nokuthula Akuxatshanwa.',
  ];

   static const imiphumelaMasendamakhulu =
  ['Ulapha Umuntu Onamasende Amakhulu, Noma Ngabe Wacushwa.',
  ];

  static const imiphumelaSafisithosami =
  ['Qeda Izilonda Esithweni Sangasese.',
  ];

  static const imiphumelaSbambeleloXYZ = 
  [
    'Ulala Nomuntu Wesifazane Kalula',
  ];

  static const imiphumelaAkondle =
  [
    'Lomuthi Wenza Ukuthi Ubaba Wengane Yakho Ayondle, Ayikhathalele Okwendlovu Ikhathalele Ichwane Layo.',
  ];

  static const imiphumelaNkomemnandi =
  ['Uyashishabhe Ngaphansi.',
  ];

  static const imiphumelaMzimboshisayo =
  ['Uyashishabhe Umzimba Wakho.',
  ];

  static const imiphumelaSkhundla =
  ['Uthola Isikhundla Emsebenzini',
  ];

  static const imiphumelaQedacala =
  ['Uqeda Icala',
  ];

  static const imiphumelaVulidloziXXX =
  ['Vula Idlozi Lakho',
  ];

  static const imiphumelaNgikufunala =
  ['Isikhafulo Sokudonsa Lona Omthandayo.',
  ];

  static const imiphumelaIntandoyasendlini =
  ['Umangabe Umdlisile Owakwakho Ngalomuthi Uhlangene Nensila Yakho Yonke, Uthanda Wena Kuphela. Akabi Yisicefe.',
  ];

   static const imiphumelaAngathinyaka =
  ['Owenze Ucansi Naye, Uvele Athande Wena Kuphela, Angathinyaka.',
  ];

  static const imiphumelaUzozibonela =
  ['Uzomangaliseka',
  ];

  static const imiphumelaUyangikweleta =
  ['Okukweletayo Uyibuyisa Eshesha Imali Yakho. Ungasebenzisa No \'Abakhokhe\' Ukuqinisekisa Ukuthi Uyayithola Imali Yakho.',
  ];

  static const imiphumelaIsikhafuloBMW =
  ['Ikuqoma Ithanda Ingathandi.',
  ];

  static const imiphumelaCuphasdumbu =
  ['Uyaziphindiselela Obulewe',
  ];

  static const imiphumelaUhlangalwabo =
  ['Uhlanga Lwabesifazane',
  ];

  static const imiphumelaHlola =
  ['Uwusebenzisa Uma Ufuna Ukubuyisa Ingilosi Yakho Yokuhlola.',
  ];

  static const imiphumelaShayizitha =
  ['Ukugadlayo Udabula Amafu.',
  ];

  static const imiphumelaDonsabafazi =
  ['Udonsa Abafazi',
  ];

  static const imiphumelaKhulumisidlozi =
  ['Enza Idlozi Lakho Likhulume.',
  ];


  static const imiphumelaNgfunumsebenzi =
  ['Owomuntu Ofuna Umsebenzi. Engathi Ungathola Nomuthi Ekuthiwa \'U-Qashwa\' Uma Unawo Amandla, Yebo Kuyasiza Kodwa Akuphoqi.',
  ];


  static const imiphumelaYakhumuzi =
  ['Uyokunihlanganisa Ninowakwakho, Isebenza Ngokwenza Ucansi.',
  ];

  static const imiphumelaQinaXXX =
  ['Owokuqina',
  ];

  static const imiphumelaNqobazitha =
  ['Usatshwa Izitha Zakho.',
  ];

  static const imiphumelaCuphathuna =
  ['Uyaziphindisesela Loyo Oshoniwe Umangabe Ufe Ngesandla Somuthu Noma Somthakathi.',
  ];

  static const imiphumelaLumbanxeba =
  ['Owoku Lumba Inxeba Uma Ugwaziwe Noma Udutshuliwe.',
  ];

  static const imiphumelaWeqiziko =
  ['Uma Ungumuntu Wedlozi Kade Ulale Nomuntu Ongamlobolanga '
  'Noma Ongakulobolanga, Fanele Usebenzise Lomuthi Emva Kokuya '
  'Ocansini Ukuze Idlozi Lakho Lingeke Ligcole.',
  ];

  static const imiphumelaPhindisa =
  ['Lomuthi Ulungele Umuntu Othole Into Abamthakatha Ngayo Egcekeni, Lomuthi Uyibuyisela Emuva Leyonto.',
  ];

  static const imiphumelaAmafuthebhanoyi =
  ['Enza Isibunge Sakho Sisebenze Ngampela.',
  ];

  static const imiphumelaThandekaABC =
  ['Uthandeka Ngendlela Exakayo Ebantwini.',
  ];

  static const imiphumelaSusaschithoXXX =
  ['Siphelanya Isichitho, Kubuye Nokuhlonipheka.',
  ];
  static const imiphumelaBolisa = ['Uyabolisa Okubi Okwenziwe Noma Okwenziwa Kuwe.',];

   static const imiphumelaOshimileXXX = ['Baziletha Bona Abesifazane Kuwe.',];

  static const imiphumelaMhlonipheni = ['Uhlonipheka Okwangampela',];

  static const imiphumelaMitha = ['Khulelwa Noma Isinya Sakho Singabambi.',];

  static const imiphumelaOlalwayisilwane = ['Umangabe Uvamise Ukuphupha Ulala '
  'Nomuntu Ekubeni Usumdala, Usuke Ulalwa Isilwane. Lomuthi Ubalekisa Lesosilwane.',];

  static const imiphumelaOwezilonda = ['Uqeda Izilonda Emzimbeni.',];
  static const imiphumelaOwezishimane = ['Mawukaqe Ungaqonywa Nhlobo Uqonywa Ngokushesha.',];
  static const imiphumelaInkingineswane = ['Isilwane Sibaleka Singasabheka Emuva.',];

  static const imiphumelaChathanje = ['Oweqolo Nesinya',];

  static const imiphumelaProfetha = ['Uma Ufuna Ukuprofetha Sebenzisa Lomuthi. Ungakhohlwa Ukuthandaza Ngo 00:00 Ebusuku Izinsuku Ezintathu.',];

  static const imiphumelaAntibiotics = ['Uzifo Zonke',];

  static const imiphumelaYonezizwe = ['Uqeda Izizwe',];

  static const imiphumelaOsheshachame = ['Wenza Uchame Emva Kwesikhathi Eside Ocansini',];

  static const imiphumelaMfonele = ['Mengabe Umfonela Ukhothe Lomuthi Uzokuthanda, Uzochazeka, Uzokufuna.',];

  static const imiphumelaMotengantshontshwa = ['Imoto Yakho Ayintshontshwa',];

  static const imiphumelaBuyisokwebiwe = ['Kuyabuya Okwebiwe',];

  static const imiphumelaMthondowothando = ['Intando Esetshenziswa Abesilisa Engena Ngocansi.',];

  static const imiphumelaAsizwisane = ['Umuntu Ulala Naye Ningathandani.',];

  static const imiphumelaUmphuphutho = ['',];

  static const imiphumelaQedizizwe = ['Umangabe Umuntu Evele Ahayize Esithubeni Nje, Lomuthi Uziqeda Nya Lezo Zizwe.',];

  static const imiphumelaGezumndeni = ['Ulwa Nokubi Okuhlasela Umndeni, Ukudala Izinxabano Emndenini.',];

  static const imiphumelaBethela = ['Bethela Umuzi Wakho Ingangeni Into Eyisandla Somthakathi.',];

  static const imiphumelaMlandengesthombesakhe = ['Indoda Noma Intombi Oyifunayo Uzoyithola Uma Ungathola Impahla Noma Isithombe Sayonje.',];

  static const imiphumelaSokalamanyala = ['Igama Lalomuthi Lusho Into Ewentayo.',];

  static const imiphumelaSesheli = ['Uqonywa Ngapha Nangapha',];

  static const imiphumelaGezelesgangeni = ['Umangabe Izinto Zakho Zivalwe Amathunzi Amnyama Ziyavuleka.',];

  static const imiphumelaZiphuzelenje = ['Geza Ingaphakathi Lakho',];

  static const imiphumelaLiyavukala = ['Ipipi Liyavka Lana',];

  static const imiphumelaDonsuthandoXYZ = ['Uma Uyintombi Ushelwa Ngaphanangapha, Uma Uyinsizwa Uqonywa Ngaphanangapha.',];

  static const imiphumelaMkhontowezitha = ['Lomuthi Ugovuzwa Ngomkhonto. Uhlehlisa Izinto Zezitha Zakho Egcekeni.',];

  static const imiphumelaMayibamanzintombi = ['',];

  static const imiphumelaPhumaschitho = ['Siphuma Siphele Nya Isichitho.',];

  static const imiphumelaSkhafulo123 = ['Udonsa Leyontombi Noma Insizwa Oyikhafulile.',];

  static const imiphumelaMfuyeningi = ['Wenza Imfuyo Yakho Yande Ngalendlela. Ikhuluphale, Izale Amawele.',];

  static const imiphumelaSondezidlozi = ['Usondezidlozi Lakho Noma Ngabe Beselikuflatheli.',];

  static const imiphumelaNgiseliwe = ['Umangabe Unenkinga Yokuhlezi Useliwe Lomuthi Uzokuqeda Loko.',];

  static const imiphumelaMubuyise = ['Isithandwa Sakho Sibuya Sigijima, Noma Ngabe Besihlukaniswe Yini.',];

  static const imiphumelaMalungabaleki = ['Wenza Ukuthi Ungabalekelwa Imali.',];

  static const imiphumelaMdayisiwecala = ['Uphephisa Umdayisa Wecala Kwabomthetho, Nasezitheni.'];

  static const imiphumelaLabongidlozi = [
    'Owokuceba Nempumelelo Lomuthi Usebenzisa Idlozi Lakho Hhayi Ukuthwala Noma Izilwane. '
    '1)udinga lezinto ezilandelayo,inkukhu emhlophe inyama yangaphakathi usu itray elidayiswa ezitolo umlotha wezinkuni hhayi owamaplangwe, ' 
    'u"Labongidlozi" (lomuthi). 2) Emsamu uzosho ukuthi upha loyo owayekuthanda imbabhama(inkukhu) emhlophe kodwa angabancishi abanye nabo usazobenzele '
    'emva kwezinyanga ezintathu ngamunye ngamunye abalinde, ungaceli lutho uyena omenzelayo ozokupha. Bazokuphendula ngezindlela ezingafani, abanye ngeke '
    'bakuphendule ngoba abanandaba nawe, abanye bazokwenzela okuhle, kodwa isigcino bazovumelana ngokukuphumelelisa ezifisweni zakho ngoba wena uzobe wenze '
    'into eyabahlula bona besaphila. Umuthi uzowusokha ko-20L ababili, kubona bobabili ufaka ispoon somlotha wezinkuni hhayi amaplangwe, kowokuqala ufaka '
    'udoti wengingila yenkukhu nesikhumba sayo nosu inyama uyifake ikuhlaza, kowesibili ufaka inyongo yenkukhu. 3) Uthatha lomuthi ofake kuwo udoti '
    'wengingila uhambe uyogeza ngaphandle kwegceke usho nkathi ugeza ukuthi azicace izinto zakho, bese uqedile ungasuli ungabheki emuva. ngakusasa uqale '
    'ugqume uphalaze ngalomuthi onenyongo izinsuku ezintathu. Mhla ubuya ukuyogeza kukhona into oyoyiphupha. 5) Inkukhu Idliwa umngeni wakho kuphela '
    'iyabhoyilwa, wenze notshwala besizulu. Izinsipho zotshwala ungazilahli uzifake ebhakedeli elivaleka thaqa zihlale khona zivaliwe kungaphela izinyanga '
    'zingaboli. Ungazomisi zigcine zimanzi, lezinsipho ezokugquma noma ngabe inini mawukhumbule, ungazifaka esiwashweni noma emthini ogquma ngawo.'

    '\nMengabe seziphelile izinyanga ezintathu dade wenze lokhu uzokhetha omunye oyedwa osewashona wenzele yena ngalendlela ekuchazwe ngayo, uphinde wenzele nomunye '
    'emva kwezinye izinyanga ezintathu njalo njalo. Uzozwa emva kweminyaka emibili kuzohamba kuhambe bathi uthwele kanti cha idlozi lakho ilona elobe selikuphakamisa. '
    'Loku uyakwenza ekhaya/kwakho noma ngabe awunawo umsamu, nomangabe umuzi awubikiwe, noma ngabe abantu abadala abangenisiwe egcekeni. Uzoloku Upha Idlozi Lakho '
    'Ngamunye Ngamunye Njalo Emva Kwezinyanga Ezintathu. Ungaze ufune ukwenza into enkulu yenza okungangamandlakho.',];

  static const imiphumelaAbakhokhe =
  [
    'Okukweletayo Ukhokha Ethanda Engathandi. Ungasebenzisa No \'Uyangikweleta\' Ukuqinisekisa Ukuthi Uyayithola Imali Yakho.'
  ];

  static const imiphumelaKuthole = 
  [
    'Uthola Leyonto Oyifunayo Ebantwini Abaningi.',
  ];

  static const imiphumelaYekisiwunga = 
  [
    'Owokuyekisa umuntu obhema iwunga, uwushunqisa emalahleni ngenkathi elele ebhemile uyoyigcina "nci".',
  ];

   static const imiphumelaWozanibathengi = 
  [
    'Ulanda Abathengi Ngalendlela',
  ];

  static const imiphumelaAlingathethwa = 
  [
    'Qeda Icala Lingathethwa',
  ];

  static const imiphumelaVikasbhamu = 
  [
    'Gqihela Isibhamu',
  ];

  static const imiphumelaLahlokubiXXX = 
  [
    'Ulungele Umuntu Othathwa Kancane Abanye Abantu.',
  ];

  static const imiphumelaBohlasisu = 
  [
    'Umangabe Ukhukhunyalelwa Isisu Lomuthi Uyasibohlisa.',
  ];

  static const imiphumelaIsibungeXXX = 
  [
    'Sikhanda Isiphithiphithi Sabathengi.',
  ];

  static const imiphumelaHambamthakathi = 
  [
    'Okuthakathayo Endaweni Uyahamba, Siyamxosha La.',
  ];

  static const imiphumelaMaqaqabebophile = 
  [
    'Kuqaqeka Izinto Zakho Noma Bekuthakathile.',
  ];

  static const imiphumelaAzwewenefonini = 
  [
    'Uzwangawe Efonini',
  ];

  static const imiphumelaWebeleni = 
  [
    'Buyisa Okwebiwe',
  ];

  static const imiphumelaMpilemnandi = 
  [
    'Izinhlela Zakho Zibamhlaphe, Kuqaqeke Amafindo.',
  ];

  static const imiphumelaBabambanokwezinja = 
  [
    'Umakwenzekile Isithandwe Sakho Salala Nomunye Umuntu, Babambana Njenge Zinja Uma Zenza Ucansi.',
  ];

  static const imiphumelaOwenhlanhlaXXX = 
  [
    'Uletha Inhlanhla',
  ];

  static const imiphumelaVukanduku = 
  [
    'Uvusa Ipipi Umangabe Ungasaqhanyelwa.',
  ];

  static const imiphumelaQhanyelwa = 
  [
    'Uvusa Ipipi Umangabe Ungasaqhanyelwa.',
  ];

  static const imiphumelaBohlasisuXXX = 
  [
    'Uma Ukhukhunyalelwa Isisu Siyehla.',
  ];

  static const imiphumelaKufogadlayo = 
  [
    'Okuthakathayo Uyafa',
  ];

  static const imiphumelaQaqabethakathile = 
  [
    'Izinto Zakho Ziqaqeka Noma Bekuthakathile.',
  ];

  static const imiphumelaQedanduna = 
  [
    'Izinduna Ziphelanya Ebusweni Bakho.',
  ];

  static const imiphumelaNqobacala = 
  [
    'Owokuwina Icala',
  ];

  static const imiphumelaMndenozwanayo = 
  [
    'Wenza Umndeni Uzwane, Ngokugeza Idlozi Elibi Nelihle Womabili Afike Kini Emahle. Kuphela Inzondo Nengxabano Emndenini, Kunaloko Umndeni Uthandane. ',
  ];

  static const imiphumelaHambamoyombi = 
  [
    'Uxosha Imimoya Emibi',
  ];

  static const imiphumelaUgezinkomo = 
  [
    'Uyayihlanganisa Inkomo Yowesifazane, Ibe-Tight.',
  ];

  static const imiphumelaUhlezekhathele = 
  [
    'Wenza Umzimba Uqaqeke Mengabe Uhlezi Ukhathele.',
  ];

  static const imiphumelaVulasfuba = 
  [
    'Umangabe Unesifuba Esicinanayo Noma Une-Astma Siyavuleka.',
  ];

  static const imiphumelaMawuyitoho = 
  [
    'Wenza Uqashwe Mengabe Uyiqoho. Ulungele Umuntu Osesebenza Vele Kodwa Oyitoho.',
  ];

  static const imiphumelaMthondowehhashi = 
  [
    'Liyagcotshwa Ipipi Bese Liyakhula.',
  ];

  static const imiphumelaBulalalumbo = 
  [
    'Ubulala Lonke Uhlobo Lwelumbo.',
  ];

  static const imiphumelaQedazintwala = 
  [
    'Uqeda Izintwala Ozithole Ngocansi.',
  ];

  static const imiphumelaIsibungeSomsebenzi = 
  [
    'Uletha Imisebenzi Kuwena, Lomuthi Ulungele Umuntu Ozisebenzayo, Ofuna Abantu Abafuna Umsebenzi/Usizo Lwakhe.',
  ];

  static const imiphumelaPholisazilonda = 
  [
    'Upholisa Izilonda',
  ];

  static const imiphumelaQedibhande = 
  [
    'Uqeda Lesifo Ekuthiwa Ibhande. Esiyizilonda Ezijikeleza Umzimba, Meke Zahlangana Uyafa.',
  ];
  

  static const imiphumelaTholamawele =
  ['Ukhulelwa Amawele Noma Umithisa Amawele.',
  ];

  static const imiphumelaKhulelwa =
  ['Ukhulelwa Nomangabe Isinya Sakho Singabambi.',
  ];

  static const imiphumelaSbambeleloXXX = 
  [
    'Siletha Abantu Besifazane',
  ];

  static const imiphumelaUbelethiswane = 
  [
    'Uma wabelethiswa isilwane kwavalwa izinto zakho, Thola Lomuthi.',
  ];

  static const imiphumelaGezamabhadi = 
  [
    'Ugeza Amabhadi.',
  ];

  static const imiphumelaIsigqabosothando = 
  [
    'Senza Uthandeke Kakhulu',
  ];

  static const imiphumelaOzalwembethe = 
  [
    'Umawazalwa Umbethe Kubuya Inhlanhla Yakho.',
  ];

  static const imiphumelaNgavalwa = 
  [
    'Owomuntu Owavalwa, Ngabe Isesigodlweni, Isebhisinisini, Isemalini, Noma Isemsebenzini.',
  ];

  static const imiphumelaNdukengami = 
  [
    'Uvusa Ipipi Elingavuki',
  ];

  static const imiphumelaWisanduku = 
  [
    'Uwisa Ipipi Kozama Ukulala Nowakwakho. Lomuthi Wenza Leyondoda Ingaqhanyelwa Nhlobo Mayithi Ilala Naye.',
  ];

  static const imiphumelaMthintabemanzi = 
  [
    'Intombi Oyithintile Ikufuna Ocansini, Ubamanzi Ngesikhathi.',
  ];

  static const imiphumelaXoshaskhova = 
  [
    'Owokuxosha Iskhova Egcekeni.',
  ];

  static const imiphumelaGadamoto = 
  [
    'Ugada Imoto Yakho Ukuze Ingantshontshwa',
  ];

  static const imiphumelaGovuEladlumthakathi = 
  [
    'Izinja Zakho Ziyamudla Oze Kabi Egcekeni Lakho, '
    'Nomangabe Umthakathi Nomangabe Isela, Nomangabe '
    'Uhlobo Luni Lwesitha.',
  ];

  static const imiphumelaUngathakatheki = 
  [
    'Awuthakatheki Mawugcabe Ngalomuthi, Ungawunela Umndeni Wonke, '
    'Uthatha Iminyaka Ukuphela Egazini. Mengabe Niwumndeni Fanele Nigcatshwe Yingane '
    'Encane (5-8 years) Ekhona Ekhaya. Yona Igcatshwe Omdala Okhona Ekhaya.'
    ' Mengabe Ingekho Ingane Emndenini Kulungile Anganigcaba Omunye Wenu.',
  ];

  static const imiphumelaBambislwane = 
  [
    'Ngokuchela Usibamba Ngqi Isilwabe Abathakatha Ngaso Egcekeni',
  ];

  static const imiphumelaBhekwayidlozi = 
  [
    'Idlozi Lakho Lihlezi Likubhekile Lapho Ohamba Khona.',
  ];

  static const imiphumelaTholokufunayo = 
  [
    'Idlozi Lakho Lihlezi Likubhekile Lapho Ohamba Khona.',
  ];

  static const imiphumelaQedaslumo = 
  [
    'Uqeda Islumo.',
  ];

  static const imiphumelaZwakalekhaya = 
  [
    'Wenza Into Oyikhulumayo Uzwakale Ekhaya Wonke Umuntu Avumelane Nawe.',
  ];

  static const imiphumelaZingaphakathi = 
  [
    'Uqeda Izilonda Ezingaphakathi.',
  ];

  static const imiphumelaShelenezizwe = 
  [
    'Uqonywa Yintombi Enezizwe Lana. Othi Uqhamuka Ibe Ifela Amathe Phansi.',
  ];

  static const imiphumelaLahliswane = 
  [
    'Ulungele Umuntu Ohluhlwa Yisilwane, Lomuthi Owokuyosilahla, Ukwenzela Umfuleni Ohambayo.',
  ];

  static const imiphumelaQedisayinasi = 
  [
    'Umangabe Unenkinga Yamakhala Ahlezi Evalekile Lomuthi Uzokusiza.',
  ];
}